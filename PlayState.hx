...
class PlayState extends MusicBeatState
{
	...
	public static var aiStrums:FlxTypedGroup<FlxSprite> = null;
	...
	public static var firstSong:Bool = true;
	public var chaosmode:Bool = false;
	var chaosmodeTxt:FlxSprite;
	...
	override public function create()
	{
		...
		SaveData.middlescroll = FlxG.save.data.middlescroll;
		SaveData.deathnotes = FlxG.save.data.deathnotes;
		SaveData.chaos = FlxG.save.data.chaos;
		SaveData.ghosttaping = FlxG.save.data.ghosttaping;
		SaveData.downscroll = FlxG.save.data.downscroll;
		SaveData.antimash = FlxG.save.data.antimash;
		SaveData.offsetx = FlxG.save.data.offsetx;
		SaveData.offsety = FlxG.save.data.offsety;
		...
		{
			case 'tutorial':
				dialogue = CoolUtil.coolTextFile('assets/data/tutorial/dialogue.txt');
			case 'bopeebo':
				dialogue = CoolUtil.coolTextFile('assets/data/bopeebo/dialogue.txt');
			case 'fresh':
				dialogue = CoolUtil.coolTextFile('assets/data/fresh/dialogue.txt');
			case 'dadbattle':
			...
			case 'roses':
			    if (PauseSubState.skipcheat == true)
				  dialogue = CoolUtil.coolTextFile('assets/data/roses/rosesCheatDialogue.txt');
				else
				  dialogue = CoolUtil.coolTextFile('assets/data/roses/rosesDialogue.txt');
			case 'thorns':
			    if (PauseSubState.skipcheat == true)
				  dialogue = CoolUtil.coolTextFile('assets/data/thorns/thornsCheatDialogue.txt');
				else
				  dialogue = CoolUtil.coolTextFile('assets/data/thorns/thornsDialogue.txt');
		}

		// thank you ninjamuffin for contributing to my pain and suffering by making this not switch case
		switch (SONG.song.toLowerCase())
		{
		...
		{
		  case 'gf' | 'gf-pixel' | 'gf-car' | 'gf-christmas':
		  {
		...
		scoreTxt = new FlxText(healthBarBG.x + healthBarBG.width - 690, healthBarBG.y + 30, 0, "", 20); // lol funny nubmer!!1
		scoreTxt.setFormat("assets/fonts/vcr.ttf", 19, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		add(scoreTxt);

		songTxt = new FlxText(healthBarBG.x + healthBarBG.width - 920, healthBarBG.y + 30, 0, "", 20);
		songTxt.setFormat("assets/fonts/vcr.ttf", 19, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.BLACK);
		...
		var tex = FlxAtlasFrames.fromSparrow('assets/images/portraits/chaosmode.png', 'assets/images/portraits/chaosmode.xml');

		chaosmodeTxt = new FlxSprite(0,0);
		chaosmodeTxt.frames = tex;
		chaosmodeTxt.animation.addByPrefix('idle', "anim", 24);
		chaosmodeTxt.animation.play('idle');
		chaosmodeTxt.screenCenter();
		chaosmodeTxt.scrollFactor.set();
		chaosmodeTxt.antialiasing = true;
		...
		if (isStoryMode)
		{
			switch (curSong.toLowerCase())
			{
				...
				case 'thorns':
					schoolIntro(doof);
				case 'tutorial' | 'dadbattle' | 'fresh' | 'bopeebo':
					schoolIntro(doof);
				default:
					startCountdown();
			}
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (firstSong && !endedSong || !isStoryMode) // only do arrow animation for first song and freeplay cause yes
			{
				if (SaveData.ghosttaping){
				  babyArrow.y -= 10;
				  babyArrow.alpha = 0;
				  ...
				  {
				    babyArrow.y -= 30;
				    FlxTween.tween(babyArrow, {y: babyArrow.y + 30}, 1, {type: FlxTweenType.PINGPONG, ease: FlxEase.quadInOut, startDelay: 0 + (0.2 * i)});
				  }
				}
				else
				{
			...
			if (SaveData.middlescroll)
			{
				...
				*/
				...
				aiStrums.forEach(function(spr:FlxSprite)
			    {
				     spr.visible = false;
			    });
			}
	...
	override public function update(elapsed:Float)
	{
		...
		var isChaos = (SaveData.chaos);

		if (chaosmode == true && isChaos)
		{
		   if (health > 0)
		   {
		       if (miss > 0 || miss == 0)
		          health -= SONG.notes.length * 0.00002;
			   if (miss > 19)
		          health -= SONG.notes.length * 0.000018;
			   if (miss > 39)
		          health -= SONG.notes.length * 0.000016;
			   if (miss > 59)
		          health -= SONG.notes.length * 0.000014;
		   }
		}
		...
		scoreTxt.text = "Score:" + songScore + " | Total Misses:" + miss + " | Health:" + healthBar.percent + "% | Difficulty:" + diftext + " | Rating:" + rating;
		songTxt.text = "Song:" + SONG.song;
		...
		// get ready for a whole lotta if statements
		...
		if (curSong == 'Roses')
		{
		    switch (curStep)
			{
			     case 60, 124, 188, 252, 316, 380, 444, 508, 572, 636:
				     FlxG.camera.flash(FlxColor.WHITE, 0.4);
					 /* use at your own risk :troll:
					   camHUD.alpha = 0;
					   FlxTween.tween(camHUD, {alpha: 1}, 0.4, {ease: FlxEase.circIn});
					 */
			}
	    }
		...
		if (generatedMusic)
		{
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (daNote.mustPress) // check out kadedev
				{
				...
				if (daNote.isSustainNote && !daNote.mustPress || daNote.isSustainNote && !daNote.canBeHit || daNote.isSustainNote && daNote.wasGoodHit || daNote.isSustainNote) // pain
			    {
				...
				if (SaveData.downscroll)
				{
				   if(daNote.isSustainNote)
				   {
						if(daNote.animation.curAnim.name.endsWith('end') && daNote.prevNote != null)
							daNote.y += ((daNote.prevNote.height / 1.05) * (0.37 * 1.6)) + (0.1 * SONG.speed); // i dont want to talk about this value
				...
				// regular 				daNote.y = (strumLine.y - (Conductor.songPosition - daNote.strumTime) * (0.45 * FlxMath.roundDecimal(SONG.speed, 2)));
				// WIP interpolation shit? Need to fix the pause issue
				// daNote.y = (strumLine.y - (songTime - daNote.strumTime) * (0.45 * PlayState.SONG.speed));

				if ((daNote.mustPress && daNote.tooLate && !SaveData.downscroll || daNote.mustPress && daNote.tooLate && SaveData.downscroll) && daNote.mustPress)
					{
	...
	private function popUpScore(strumtime:Float):Void
	{
		...
		if (noteDiff > Conductor.safeZoneOffset * 0.81)
		{
			daRating = 'shit';
			score = 50;
			miss += 1; // copy needed?
		...
		else if (noteDiff > Conductor.safeZoneOffset * 0.67)
		{
			daRating = 'bad';
			score = 100;
			health -= 0.014;
		}
		else if (noteDiff > Conductor.safeZoneOffset * 0.32)
		{
			daRating = 'good';
			score = 200;
		}
		...
		rating.screenCenter();
		rating.x = SaveData.offsetx;
		rating.y = SaveData.offsety;
	...
	function missAnim(direction:Int = 1):Void
	{
		if (!boyfriend.stunned)
		{
			if (combo > 5)
	...
	function shitNote(direction:Int = 1):Void
	{
		if (!boyfriend.stunned)
		{
			// nothing lmao
